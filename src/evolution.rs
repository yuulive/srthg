...
pub fn population_from_multilevel_sub_populations<Gene, IndexFunction, Data>(
    ...
    operations: Vec<Operation<Gene, Data, IndexFunction>>
) -> Population<Gene> 
where
...
Data: Clone + Send + 'static
{
    ...
    populations_from_existing_multillevel(populations, levels, sub_populations_per_level, &data, iterations_on_each_population, &operations)
}
...
Standard: Distribution<Gene>,
IndexFunction: Clone + Send + Sync + Fn(&Agent<Gene>, &Data) -> isize + 'static,
Data: Clone + Send + Sync + 'static
{
    ...
    populations_from_existing_multillevel(populations, 1, sub_populations_per_level, data, iterations_on_each_population, operations)
}
    ...
    for level in (0..levels).rev() {
        ...
        for _ in 0..number_of_new_populations {
            ...
            new_populations.push(
                cull_lowest_agents(
                    run_iterations(population, iterations_on_each_population, data, operations),
                    cull_proportion,
                    1
                )
            );
